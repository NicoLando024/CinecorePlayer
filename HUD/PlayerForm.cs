...
namespace CinecorePlayer2025
{
    ...
    public sealed class PlayerForm : Form
    {
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            // --- Modalità foto: solo frecce + ESC
            if (IsPhotoMode)
            {
                if (keyData == Keys.Left) { ShowPrevImage(); return true; }
                if (keyData == Keys.Right) { ShowNextImage(); return true; }
                if (keyData == Keys.Escape) { SafeStop(); ShowLibrary(); return true; }
                return base.ProcessCmdKey(ref msg, keyData);
            }

            // --- UI navigabile (Settings / Libreria): DPAD + OK + ESC
            if (inUi)
            {
                if (keyData == Keys.Left) { RemoteMove("left"); return true; }
                if (keyData == Keys.Right) { RemoteMove("right"); return true; }
                if (keyData == Keys.Up) { RemoteMove("up"); return true; }
                if (keyData == Keys.Down) { RemoteMove("down"); return true; }
            ...
            // --- Playback hotkeys del player (solo senza modifier)
