...
namespace CinecorePlayer2025
{
    ...
    // ===================== BASE PER LE MODALI SETTINGS / CREDITS =====================
    internal abstract class ModalBaseV2 : UserControl
    {
        private Label _titleLabel = null!;
        private Panel _header = null!;

        protected CardPanel Card = null!;
        ...
        public string TitleText
        {
            ...
            set { if (_titleLabel != null) _titleLabel.Text = value?.ToUpperInvariant() ?? ""; }
        }

        protected float DpiScale => DeviceDpi > 0 ? DeviceDpi / 96f : 1f;
        ...
        protected ModalBaseV2()
        {
            ...
            {
                using var p = new Pen(Theme.Border, 1f);
                pe.Graphics.DrawLine(p, 0, _header.Height - 1, _header.Width, _header.Height - 1);
            };
            Card.Controls.Add(_header);

            _titleLabel = new Label
            {
                Dock = DockStyle.Right,
                Width = 420, // più largo per non tranciare "IMPOSTAZIONI" / "CREDITI"
        ...
        protected virtual int GetMinCardWidth(float s) => (int)(900 * s);
        protected virtual int GetMaxCardWidth(float s) => (int)(1400 * s);
        protected virtual int GetMinCardHeight(float s) => (int)(600 * s);
        protected virtual int GetMaxCardHeight(float s) => (int)(900 * s);
        ...
        private void LayoutCard()
        {
            ...
            int margin = (int)(16 * DpiScale);

            int minW = GetMinCardWidth(DpiScale);
            int maxW = GetMaxCardWidth(DpiScale);
            int minH = GetMinCardHeight(DpiScale);
            int maxH = GetMaxCardHeight(DpiScale);

            int cardW = Math.Max(minW, Math.Min(maxW, hostW - margin * 2));
            int cardH = Math.Max(minH, Math.Min(maxH, hostH - margin * 2));
    ...
    internal sealed class SettingsModal : ModalBaseV2
    {
        ...
        {
            // Precarica tutte le pagine in modo che l'esterno possa già usarle
            EnsurePageGeneral();
            EnsurePageMadvr();
            EnsurePageLavVideo();
            EnsurePageLavAudio();
        ...
        private void BuildLayout()
        {
            ...
            hostSplit.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 170)); // colonna sinistra come screenshot
            hostSplit.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
            BodyPanel.Controls.Add(hostSplit);
            ...
            _bottomButtonsHost.Controls.Add(bottomLine);

            // nav buttons host --> FILL tra logo e footer, scrollabile
            ...
            // ORDINE NOTE: con Dock=Top, l'ultimo aggiunto finisce in alto
            _bottomButtonsHost.Controls.Add(_btnApply);   // va in basso
            _bottomButtonsHost.Controls.Add(_btnSpacer);  // spazio
            _bottomButtonsHost.Controls.Add(_btnClose);   // va in alto (visivamente sopra Apply)

            // Creo i bottoni nav (stesso ordine logico che vuoi)
            string[] navItems =
            {
                "Generali",
                "madVR",
            ...
            var rightTopLine = new Panel
            {
                Dock = DockStyle.Top,
                Height = 1,
                BackColor = Theme.Border
            };
            _rightBorder.Controls.Add(rightTopLine);
        ...
        // applichiamo tutte le property page nella pagina visibile (LAV/MPC ecc)
        ...
        private void SetNavSelected(int index)
        {
            ...
            // madVR: appena apro la tab madVR deve avviarsi in automatico
        ...
        private void EnsurePageGeneral()
        {
            ...
            grid2.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 70f)); // colonna sx grande
            grid2.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 30f)); // colonna dx stretta
            page.Controls.Add(grid2);

            // colonne verticali con FlowLayoutPanel
            ...
            var rightCol = new FlowLayoutPanel
            {
                ...
                AutoSizeMode = AutoSizeMode.GrowAndShrink,
                BackColor = Theme.Panel,
                Margin = new Padding(12, 0, 0, 0), // piccolo distacco orizzontale dal bordo sx
                Padding = new Padding(0)
            };
            ...
            // helper BOX (outline 1px, titolo+sub, body dinamico)
            OutlinePanel Box(Control parent, string title, string subtitle)
            {
                var box = new OutlinePanel
                {
                    ...
                    Margin = new Padding(0, 0, 0, 12) // distanza verticale tra box
                };
                ...
                sub.Location = new Point(0, header.Bottom + 0);
                ...
                // allarga il box alla larghezza della colonna che lo ospita
                parent.Resize += (_, __) => { box.Width = parent.ClientSize.Width; };
                box.Width = parent.ClientSize.Width;

                parent.Controls.Add(box);
                return box;
            }

            // ---- SINISTRA: Algoritmi madVR ----
            var boxAlgo = Box(leftCol, "Algoritmi madVR",
                "Profili madVR per upscaling / downscaling / refinement. Scegli quale profilo usare per ciascuna categoria.");
            var bodyAlgo = (Panel)boxAlgo.Tag;

            // tabellina label+combo
            ...
            // ---- SINISTRA: HDR ----
            var boxHdr = Box(leftCol, "madVR — HDR",
                "Come gestire i contenuti HDR.");
            var bodyHdr = (Panel)boxHdr.Tag;

            _hdrAuto = UiKit.MakeRadio("Auto");
            _hdrAuto.Checked = true;
            _hdrPass = UiKit.MakeRadio("Passthrough HDR direttamente al display");
            _hdrTone = UiKit.MakeRadio("Converti HDR → SDR (tone mapping)");
            _hdrLut = UiKit.MakeRadio("HDR → SDR usando LUT 3D esterna");

            bodyHdr.Controls.Add(_hdrAuto);
            _hdrPass.Top = _hdrAuto.Bottom + 2;
            bodyHdr.Controls.Add(_hdrPass);
            _hdrTone.Top = _hdrPass.Bottom + 2;
            bodyHdr.Controls.Add(_hdrTone);
            _hdrLut.Top = _hdrTone.Bottom + 2;
            bodyHdr.Controls.Add(_hdrLut);

            // ---- SINISTRA: Frequenza monitor ----
            var boxFps = Box(leftCol, "Frequenza monitor",
                "Cambia il refresh del display durante la riproduzione (film vs desktop/sport).");
            var bodyFps = (Panel)boxFps.Tag;
            ...
            bodyFps.Controls.Add(_fpsAuto);
            _fps60.Top = _fpsAuto.Bottom + 2;
            bodyFps.Controls.Add(_fps60);
            _fps24.Top = _fps60.Bottom + 2;
            bodyFps.Controls.Add(_fps24);

            // ---- DESTRA: Audio ----
            var boxAud = Box(rightCol, "Audio",
                "Uscita audio e passthrough bitstream.");
            var bodyAud = (Panel)boxAud.Tag;
            ...
            // ---- DESTRA: Video (player) ----
            var boxVid = Box(rightCol, "Video (player)",
                "Opzioni interne del player quando NON sto usando madVR.");
            var bodyVid = (Panel)boxVid.Tag;
    ...
    internal sealed class CreditsModal : ModalBaseV2
    {
        ...
        // questa modale è più piccola in altezza rispetto alle impostazioni
        protected override int GetMinCardWidth(float s) => (int)(600 * s);
        protected override int GetMaxCardWidth(float s) => (int)(800 * s);
        protected override int GetMinCardHeight(float s) => (int)(400 * s);
        protected override int GetMaxCardHeight(float s) => (int)(600 * s);

        private void BuildLayout()
        {
            BodyPanel.Controls.Clear();
            BodyPanel.Padding = new Padding(16);
            BodyPanel.BackColor = Theme.Panel;
            ...
            BodyPanel.Controls.Add(outline);
            ...
            _btnClose.Click += (_, __) => RaiseClose();
