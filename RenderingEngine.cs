    ...
    public sealed class DirectShowUnifiedEngine : IPlaybackEngine
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    else if (mt.formatType == FormatType.WaveEx && mt.formatPtr != IntPtr.Zero)
                    {
                        ...
                        {
                            _bitstreamActive = true; // IEC61937, AC3, DTS…
                        }
                    }
                    else
                    {
        ...
        private void TryDumpNegotiatedVideoMT()
        {
            try
            {
                ...
                var mt = new AMMediaType();
                int hr = inPin.ConnectionMediaType(mt);
                if (hr != 0 || mt.majorType == Guid.Empty)
                {
                    Dbg.Warn($"ConnectionMediaType non pronto (hr=0x{hr:X8}).");
                    return;
                }
                ...
                int w = 0, h = 0; string sub = GuidToCodecName(mt.subType);
                ...
                if (mt.formatType == FormatType.VideoInfo2 && mt.formatPtr != IntPtr.Zero)
                {
                    ...
                    sb.AppendLine($"  size: {w}x{h}  bitCount: {vih.BmiHeader.BitCount}");
                    if (_fileIsHdr) HdrTrace($"Negotiated subtype={Dbg.Hex(mt.subType)} size={w}x{h} bitCount={vih.BmiHeader.BitCount}");
                }
                ...
                if (mt != null) DsUtils.FreeAMMediaType(mt);
                SafeStatus($"Video negoziato: {w}x{h} {sub}");
            }
        ...
        private void StartMediaTypePolling()
        {
            ...
            Task.Run(async () =>
            {
                // poll fino a 2s dopo il Run: alcuni renderer negoziano tardi
                for (int i = 0; i < 10 && !cts.IsCancellationRequested; i++)
                {
                    TryDumpNegotiatedVideoMT();
                    var cf = _cachedFmt;
                    if (cf.W > 0 && cf.H > 0) break;
        ...
        {
            var cf0 = _cachedFmt;
            if (cf0.W > 0 && cf0.H > 0) return (cf0.W, cf0.H, cf0.Sub);
