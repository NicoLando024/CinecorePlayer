...
namespace CinecorePlayer2025.Engines
{
    ...
    public sealed class DirectShowUnifiedEngine : IPlaybackEngine
    {
        ...
        public DirectShowUnifiedEngine(bool preferBitstream, bool forcePcmToggle, string? preferredRendererName, VideoRendererChoice choice, bool fileIsHdr, AVCodecID srcAudioCodec)
        {
            ...
            Dbg.Log($"Engine ctor: preferBitstream={preferBitstream}, forcePcmToggle={forcePcmToggle}, preferredAR={preferredRendererName ?? "null"}, choice={choice}");
        }
        ...
        private void ConnectAudioPath()
        {
            try
            {
                ...
                var srcA = FindPin(srcSplitter, PinDirection.Output, MediaType.Audio)
                ...
                bool forcePcmGraph = !_preferBitstream && MediaProbe.IsPassthroughCandidate(_srcAudioCodec);
                    ...
                    try
                    {
                        ...
                        if (mpc != null)
                        {
                            ...
                            if (mpcIn != null && mpcOut != null)
                            {
                                ...
                                _updateCb?.Invoke();
                                return; // IMPORTANTE: non proseguire con il ramo LAV Audio
                            }
                            else
                            {
                ...
                if (aIn != null && aOut != null && rIn != null)
                {
                    ...
                    if (forcePcmGraph)
                    {
                        ...
                        {
                            // Fallback: come prima
                            hr = _graph.Connect(aOut, rIn);
                            DsError.ThrowExceptionForHR(hr);
                            Dbg.Log("Audio: LAV Audio → Renderer (fallback)", Dbg.LogLevel.Verbose);
                        }
                    }
                    else
                    {
            ...
            try
            {
                if (_lavSource is IAMStreamSelect sel)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        bool isSub = mt.majorType == MediaType.Texts || LooksLikeSubtitleName(name) || LooksLikeSubtitleSubtype(mt);
        ...
        private void TryDumpNegotiatedVideoMT()
        {
            try
            {
                ...
                if (mt.formatType == FormatType.VideoInfo2 && mt.formatPtr != nint.Zero)
                {
        ...
        public void UpdateVideoWindow(nint ownerHwnd, Rectangle ownerClient)
        {
            ...
            try
            {
                if (_mfDisplay != null)
                {
                    ...
                    {
                        _mfDisplay.SetVideoPosition(nint.Zero, ref dest);
                    }
                    _mfDisplay.RepaintVideo();
                    _lastDest = dest;
                    return;
