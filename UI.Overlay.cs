...
namespace CinecorePlayer2025
{
    ...
    public sealed class PlayerForm : Form
    {
        ...
        private void OpenFileWithDialog()
        {
            using var ofd = new OpenFileDialog
            {
                ...
                Filter =
                    "Video e audio|*.mkv;*.mp4;*.m2ts;*.ts;*.mov;*.avi;*.wmv;*.webm;*.mts;*.mp3;*.flac;*.m4a;*.aac;*.ogg;*.opus;*.wav|" +
                    "Solo video|*.mkv;*.mp4;*.m2ts;*.ts;*.mov;*.avi;*.wmv;*.webm;*.mts|" +
                    "Solo audio|*.mp3;*.flac;*.m4a;*.aac;*.ogg;*.opus;*.wav|" +
                    "Tutti i file|*.*",
        ...
        private void BuildMenu()
        {
            ...
            // opzionale: aggiungi anche una voce per la libreria
            ...
            // 1) Auto
            var hAuto = new ToolStripMenuItem("Auto (let madVR decide)", null, (_, __) =>
            {
            ...
            var m3Off = new ToolStripMenuItem("Off (immagine doppia)", null, (_, __) => { Disable3DRestoreRenderer(); });
            var m3SBS = new ToolStripMenuItem("SBS → 2D (usa EVR)", null, (_, __) => { Enable3D(Stereo3DMode.SBS); });
            var m3TAB = new ToolStripMenuItem("TAB → 2D (usa EVR)", null, (_, __) => { Enable3D(Stereo3DMode.TAB); });

            m3D.DropDownItems.AddRange(new[] { m3Off, m3SBS, m3TAB });

            // Mantieni le spunte coerenti con lo stato corrente
            ...
            var mShowPin = new ToolStripMenuItem("Telecomando (mostra PIN)", null, (_, __) =>
            {
                if (_remote != null)
                {
                    // forza la (ri)comparsa del banner con il PIN corrente
                    ShowPairingBanner(_remote.CurrentPin);
                }
            });
            ...
            {
                // Se il 3D→2D è attivo, EVR è obbligatorio.
                if (_stereo != Stereo3DMode.None && c != VRChoice.EVR)
                {
                    // Memorizza comunque la preferenza per quando disattiverai il 3D
                    _hasSavedRendererFor3D = true;
                ...
                // Se NON è madVR, spegni l'upscaling (coerente con la tua logica esistente)
            ...
            // === Preferenza uscita (PCM/Bitstream) ===
        ...
        private void OpenPath(string path, double resume = 0, bool startPaused = false)
        {
            ...
            // === NEW: se è un URL http/https prova a risolverlo in una URL media diretta ===
                ...
                (u.Scheme == Uri.UriSchemeHttp || u.Scheme == Uri.UriSchemeHttps))
            {
                ...
                if (r == null)
                {
                    _lblStatus.Text = "Questo link non è un media diretto. Usa un .m3u8/.mp4 oppure metti 'yt-dlp.exe' nella cartella del player.";
                    return;
                }
                path = r.Value.Url;
            ...
            bool hasVideo = forceHasVideo ?? (_info?.HasVideo == true || LooksLikeVideoByExt(path));
            ...
            foreach (var choice in order)
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        bool isLocal = true;
                        isLocal = Uri.TryCreate(path, UriKind.Absolute, out var uriLocal) && uriLocal.IsFile;
                        if (isLocal)
                        {
                            _pktRate.Open(path);
    ...
    internal static class WebMediaResolver
    {
        ...
        {
            // Già “diretto”? (mp4/mp3/m3u8/etc.) → usa così com’è
            if (LooksDirectMediaUrl(url)) return new Result { Url = url, ForceHasVideo = null };

            // Prova yt-dlp se presente (stessa cartella dell’exe o nel PATH)
            var ytdlp = TryFindExe("yt-dlp.exe") ?? TryFindExe("yt-dlp");
            if (ytdlp != null)
            {
                // best: prova a ottenere un singolo URL (preferisci HLS/mp4)
                // NB: --no-playlist evita liste YouTube
                var args = $"--no-playlist -f \"bv*+ba/b\" --get-url \"{url}\"";
                ...
                try
                {
                    ...
                    // Scegli una URL plausibile: prima .m3u8, poi .mp4/.webm, altrimenti ultima riga
                    string? pick = lines.FirstOrDefault(l => l.Contains(".m3u8", StringComparison.OrdinalIgnoreCase))
                                 ?? lines.FirstOrDefault(l => HasMediaLikePath(l))
                                 ?? lines.LastOrDefault();

                    if (!string.IsNullOrWhiteSpace(pick))
                        return new Result { Url = pick!, ForceHasVideo = null };
                }
                catch { /* ignora e passa al fallback */ }
            }

            // Fallback: HEAD veloce → accetta se Content-Type è audio/*, video/* o HLS
            try
            {
                using var handler = new HttpClientHandler { AllowAutoRedirect = true };
                using var http = new HttpClient(handler) { Timeout = TimeSpan.FromSeconds(4) };
                using var req = new HttpRequestMessage(HttpMethod.Head, url);
                using var res = http.Send(req); // in .NET 9 esiste Send; se non compila usa SendAsync(...).GetAwaiter().GetResult()
                ...
                if (ct.StartsWith("video/", StringComparison.OrdinalIgnoreCase) ||
                    ...
                    ct.Equals("application/vnd.apple.mpegurl", StringComparison.OrdinalIgnoreCase) ||
                    ct.Equals("application/x-mpegURL", StringComparison.OrdinalIgnoreCase))
                {
                    return new Result { Url = url, ForceHasVideo = ct.StartsWith("audio/", StringComparison.OrdinalIgnoreCase) ? false : null };
                }
            }
            catch { /* no-op */ }

            // Niente di buono trovato
            return null;
        }

        // ===== helpers =====
        private static bool LooksDirectMediaUrl(string url) => HasMediaLikePath(url) ||
        ...
        private static bool HasMediaLikePath(string url)
        {
            try
            {
                ...
                return new[] { ".mp4", ".m4v", ".webm", ".mp3", ".m4a", ".flac", ".wav", ".ogg", ".opus", ".ts", ".m2ts", ".mov", ".avi", ".wmv", ".mkv" }.Contains(ext);
            }
            catch { return false; }
        }
